...
void btns_callback(uint gpio, uint32_t events)
{
    ...
    if (current_time - last_time > DEBOUNCE_TIME)    // Verifica se ja passou o tempo do debounce para acionamento novamente do botao
    {
        if (gpio == BTN_A)
        {
            acionamento_botoes(!gpio_get(LED_GREEN), verde_on = true); //func para acionar o led VERDE, imprimir no terminal e atualizar o display
        }
        else if (gpio == BTN_B)
        {
            acionamento_botoes(!gpio_get(LED_BLUE), verde_on = false); //func para acionar o led AZUL, imprimir no terminal e atualizar o display
        }
        ...
        { 
            ...
            { 
                ...
                case '9':
                    ...
                    break;
                default:
                    desenhar(c); //func para atualizar o display com o caractere digitado
